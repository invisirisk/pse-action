...
setup_certificates() {
  ...
  if command -v docker >/dev/null 2>&1; then
    debug "Configuring certificates for Docker"
    ...
    if command -v systemctl >/dev/null 2>&1; then
      debug "Restarting Docker with systemctl"
      #run_with_privilege systemctl restart docker
      #RESTART_EXIT_CODE=$?
      debug "DEBUG: systemctl docker restart exit code: $RESTART_EXIT_CODE"
    else
      debug "Restarting Docker with service"
      #run_with_privilege service docker restart
      #RESTART_EXIT_CODE=$?
      debug "DEBUG: service docker restart exit code: $RESTART_EXIT_CODE"
